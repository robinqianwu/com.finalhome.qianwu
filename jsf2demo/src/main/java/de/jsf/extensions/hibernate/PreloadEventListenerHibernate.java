...
public class PreloadEventListenerHibernate extends DefaultPostLoadEventListener
{
    final static Logger                   logger              = Logger.getLogger(PreloadEventListenerHibernate.class);
            ...
            for ( Preload preload : preloads)
            {
                if (preload.getDomainClass().isInstance(entity))
                {
                    ...
                    catch ( LazyInitializationException e )
                    {
                        ...
                        // passiert, wenn Preloads definiert werden, welche
                        // gelichzeitig per fetch join in hql geholt werden
                    ...
                    catch ( EntityNotFoundException e )
                    {
                        ...
                        // hier passiert ev. eine
                        // javax.persistence.EntityNotFoundException ex
                        // -> wenn ein Property aus dem 2nd-Level Cache kommt,
                        // aber nicht mehr in der db exisitert (L�sung:
                        // Objekt-Graph aufr�umen, d.h. Objekte auch aus
                        // Collections l�schen anstatt nur von der Datenbank!)
                        // ->
